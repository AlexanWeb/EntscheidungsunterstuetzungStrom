                ...
                @for($i = 1; $i < 25; $i++)
                    ...
                    <div class="row">
                        @if($i===3)
                            ...
                            @for($j = 1; $j < 5; $j++)
                                <div class="col-md-6-25 border-bottom border-top text-center"  style="background-color: {{bg_color_prices($dbtm_pid['Hour_'.$i.'A_Q'.$j] - $dbt_pid['Hour_'.$i.'A_Q'.$j], $dbt_pid['Hour_'.$i.'A_Q'.$j])}}" > {{number_format($dbtm_pid['Hour_'.$i.'A_Q'.$j], 2)}} €</div>
                            @endfor
